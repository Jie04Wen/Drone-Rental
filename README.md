...
2026/08/21
   ...
   实现了 minIO 对象存储接入，实现**私有对象存储 + 业务鉴权 + 短期预签名 URL**模式
...
1. 对原型项目的代码改动
- upload() 从返回完整 URL 改为返回稳定对象键，对象键规范化，按业务类型组织（对象键由服务器生成）

      后端先根据 ID 查询数据库记录，再从记录中读取 objectKey，最后调用 signed_file_data() 签名

- 添加get_access_url()，将数据库中的**稳定对象键转换为实际访问地址**，配合 presigned_get_object() 使用，生成临时 URL

- 增加对象归属验证、路径安全检查、安全策略

  当前用户只能提交 users/{current_user_id}/业务名称/{uuid}/...      同时数据库保存逻辑同步更改

  实现业务鉴权接口：用户接口/api/user/...、管理员接口/api/admin/...、公开业务资源接口/api/drone/...和/api/comment/...

  实现MinIO 预签名 URL：鉴权通过的用户在固定时间内直接访问 MinIO。
  
      安全策略包括：
      - 不提供“传入任意对象键即可获取 URL”的通用签名接口，避免越权漏洞
      - 用户证件和资质文件必须登录后访问（通过 FastAPI 依赖注入和 JWT 实现）
      - 用户故障图片接口会检查故障单是否属于当前用户
      - 管理端接口必须具有管理员身份
      - 公开接口允许访问上架设备资源和正常展示的评价
  
- 修复故障详情及按订单查询故障记录时缺少资源归属校验的问题

- 兼容历史 /uploads/... 本地文件地址，采用“新旧存储并存”的兼容策略：

      对于历史图片，仍从本地 uploads/ 读取
      对于新图片，存入 MinIO，数据库保存 users/... 或 drones/... 对象键

2. 前端改动

   主要对js文件和vue文件进行改动，使文件上传、数据库保存和图片预览相互分离。

- 上传时传入对应 category

      POST /api/common/upload?category=具体分类（avatar/qualification/drone/comment/fault）

- 数据保存与预览分离

      表单提交时保存 res.data.objectKey，不再保存临时预签名 URL
      文件上传完成后的即时预览使用 res.data.url
      预签名 URL 失效后，可重新调用对应业务授权接口获取新的 URL

- 编辑无人机时区分：数据库对象键 imageKey 和 页面预览地址 imagePreviewUrl
- 私有文件访问调整，要求使用业务鉴权和专用接口，此外资质审核和故障审核页面不再直接拼接 MinIO 地址。
...
2026/09/02
  ...
  接入**Redis**：使用 redis‑py 封装一个 Redis 工具类 **RedisSupport** (app/redis.py) 作为增强组件存在。

      - Redis正常时：提供JWT退出失效、登录会话记录、无人机查询缓存、跨实例限流、订单防重和库存操作的分布式互斥。
      - Redis关闭或故障时:原有业务逻辑完全可用，API走MySQL和现有单进程保护逻辑，不因Redis返回500。

  不把Redis客户端散落到路由和服务中，而是建立一个职责明确的**Redis 基础设施模块** app/redis.py。
  
  当前项目是同步 SQLAlchemy，使用同步 redis.Redis

  具体内容如下：
  
      - 登录会话与退出：记录登录会话，退出时把 JWT 加入黑名单，使旧 Token 失效。
      - 无人机缓存：缓存无人机列表和详情，减少 MySQL 查询；库存或状态变化后自动使缓存失效。
      - 接口限流：限制登录、注册、下单等接口的访问频率。
      - 订单幂等：防止用户重复点击或网络重试造成重复订单。
      - 库存分布式锁：并发下单时按无人机加锁，配合 MySQL 条件更新避免超卖。
...
2026/09/08
  ...
  接入**deepseek-v4-flash-vision-exp**接口，实现**路由策略**：
  
      纯文本对话 → deepseek-v4-flash
      包含图片的对话 → deepseek-v4-flash-vision-exp
  ...
  2. 设计24小时临时图片数据库表，不把 Base64 或临时签名 URL写入聊天记录。
