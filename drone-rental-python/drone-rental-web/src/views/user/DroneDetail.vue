      ...
      <div class="detail-sidebar">
        <GlassCard class="rent-card">
          ...
          <el-button
              ...
              :disabled="!canRent || orderSubmitting"
              class="rent-btn"
              @click="handleRent"
          >
            {{ rentBtnText }}
          ...
          <div v-else-if="!authStore.isVerified" class="login-tip">
            请先完成
            <router-link to="/profile">实名认证</router-link>
          </div>
        </GlassCard>
...
const handleRent = async () => {
  if (!canRent.value || orderSubmitting.value) return

  // 同一次订单提交和重试复用同一个幂等键
  if (!orderIdempotencyKey.value) {
    orderIdempotencyKey.value = crypto.randomUUID()
  }
  ...
  try {
    ...
    const res = await createOrder({
      ...
      startDate,
      endDate,
      _idempotencyKey: orderIdempotencyKey.value
    })

    orderIdempotencyKey.value = ''

    ElMessage.success({ message: '订单创建成功', duration: 1000 })
    await router.push(`/orders/${res.data.id}/pay`)
  } catch (error) {
    // 错误已在拦截器中处理
  } finally {
