      ...
      >
        ...
        <div class="order-content" @click="goToDetail(order.id)">
          ...
          <div class="order-price">
            ...
            <span class="price-value">¥{{ Number(order.payableAmount || 0).toFixed(2) }}</span>
          </div>
        </div>
        ...
        <div class="order-footer">
          <div class="order-actions">
            ...
            <template v-else-if="order.orderStatus === 7">
              <el-button type="primary" @click="openShipmentDialog(order)">填写寄回物流</el-button>
            </template>

            <!-- 待商家收货 (status=8) -->
            <template v-else-if="order.orderStatus === 8">
              <el-button disabled>等待商家收货验机</el-button>
              <el-button @click="goToDetail(order.id)">查看物流</el-button>
            </template>
...
import {getMyOrders, cancelOrder, confirmReceive, applyReturn, submitReturnShipment, commentOrder} from '@/api/order'
...
const orderTabs = ref([
  ...
  {label: '待寄回', value: '7', count: 0},
  {label: '待商家收货', value: '8', count: 0},
  {label: '已归还', value: '4', count: 0},
  {label: '已取消', value: '5', count: 0},
  {label: '已退款', value: '6', count: 0}
])

const shipmentForm = reactive({orderId: null, expressCompany: '', expressNo: ''})
...
const commentRules = {
  ...
  content: [
    ...
    {min: 1, message: '评价内容至少1个字', trigger: 'blur'}
  ]
}
...
const openShipmentDialog = (order) => {
  Object.assign(shipmentForm, {orderId: order.id, expressCompany: '', expressNo: ''})
