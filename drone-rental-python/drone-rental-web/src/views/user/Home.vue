    ...
    <section class="hero">
      <div class="hero__content">
        ...
        <p class="hero__description">
          我们提供各类高品质无人机设备租赁服务，<br>
          满足航拍、测绘、农业、巡检等多种场景需求。
        </p>
...
import {getDroneList, getDroneStats} from '@/api/drone'
...
// 统计数据
const stats = ref([
  {value: '50+', label: '已有设备型号', icon: markRaw(Goods), color: 'primary'},
  {value: '1000+', label: '可租赁型号', icon: markRaw(TrendCharts), color: 'success'},
  {value: '500+', label: '设备品牌', icon: markRaw(User), color: 'info'},
  {value: '99%', label: '在线技术支持', icon: markRaw(Medal), color: 'warning'}
])
...
const features = [
  {
    icon: markRaw(Goods),
    title: '丰富机型',
    description: '涵盖遥感航拍、测绘、农业、电力巡检等多场景专业无人机',
    color: '#efce1a'
  },
  {
    icon: markRaw(Timer),
    title: '灵活租期',
    description: '支持按天/周/月租赁，适配短期项目与长期作业',
    color: '#1082e4'
  },
  {
    icon: markRaw(Lock),
    title: '安全保障',
    description: '设备定期检修，全程租赁风险保障',
    color: '#22c55e'
  },
  {
    icon: markRaw(Service),
    title: '专业服务',
    description: '提供设备调试、飞行指导、售后维保一站式服务',
    color: '#f5490b'
  }
]
...
const processSteps = [
  ...
  {title: '选择设备', description: '浏览设备并选择合适机型'},
  {title: '下单支付', description: '确认订单信息并完成支付'},
  {title: '收货使用', description: '签收设备开始使用'},
  {title: '归还评价', description: '使用完毕归还并反馈'}
]
...
const fetchStats = async () => {
  try {
    const res = await getDroneStats()
    const data = res.data || {}
    stats.value[0].value = (data.totalDrones || 0) + ' 款'
    stats.value[1].value = (data.availableDrones || 0) + ' 款'
    stats.value[2].value = (data.brandCount || 0) + ' 个'
    stats.value[3].value = '7×24h'
  } catch (error) {
...
.hero {
  ...
  &__title-highlight {
    color: #3b82f6;
    text-shadow: 0 0 30px rgba(155, 214, 243, 0.5);
  }
