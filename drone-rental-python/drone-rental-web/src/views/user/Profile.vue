            ...
            <div v-else class="order-list">
              <div v-for="order in orderList" :key="order.id" class="order-item"
                ...
                <div class="order-header">
                  ...
                  <StatusTag :text="getOrderStatusText(order.orderStatus)" :type="getOrderStatusType(order.orderStatus)"
                             size="small"/>
                </div>
                <div class="order-body">
                  <span class="drone-model">{{ order.droneModel }}</span>
                  <span class="order-amount">¥{{ Number(order.payableAmount || 0).toFixed(2) }}</span>
                </div>
            ...
            <el-timeline v-else>
              ...
              >
                <div class="history-item" @click="router.push(`/orders/${order.id}`)">
                  ...
                  <p>实付金额：¥{{ Number(order.payableAmount || 0).toFixed(2) }}</p>
                  <StatusTag :text="getOrderStatusText(order.orderStatus)" :type="getOrderStatusType(order.orderStatus)"
                             size="small"/>
                </div>
              </el-timeline-item>
        ...
        <GlassCard v-if="activeMenu === 'credit'" title="诚信记录">
          <div class="credit-section">
            ...
            <div class="credit-history">
              <h5>变更历史</h5>
              <div v-loading="creditLoading" class="credit-history-list">
                <div v-if="!creditRecords.length && !creditLoading" class="empty-state">
                  <el-icon :size="36">
                    <Medal/>
                  </el-icon>
                  <p>暂无诚信变更记录</p>
                ...
                <el-timeline v-else>
                  <el-timeline-item
                      ...
                      :key="record.id"
                      :timestamp="formatDateTime(record.createdTime)"
                      placement="top"
                      :type="record.afterStatus === 1 ? 'success' : 'danger'"
                  >
                    <div class="credit-record-item">
                      <h4>{{ getChangeTypeText(record.changeType) }}</h4>
                      <p>变更：{{ record.beforeStatus === 1 ? '正常' : '不良' }} →
                        {{ record.afterStatus === 1 ? '正常' : '不良' }}</p>
                      <p v-if="record.reason">原因：{{ record.reason }}</p>
                      <p v-if="record.operatorName" class="record-operator">操作人：{{ record.operatorName }}</p>
                    </div>
                  </el-timeline-item>
                </el-timeline>
              </div>
...
import {
  ...
  uploadAvatar,
  getQualification,
  getQualificationCertificateUrl,
  getAvatarUrl,
  getMyOrders,
  getMyCreditRecords
} from '@/api/user'
...
const completedOrders = computed(() => {
  return orderList.value.filter(o => [4, 5, 6].includes(o.orderStatus))
})
...
const handleAvatarUpload = async (options) => {
  ...
  try {
    ...
    const avatarPath = uploadRes.data.objectKey

    // 更新用户头像
    await updateUserInfo({avatar: avatarPath})
    const avatarRes = await getAvatarUrl()
    authStore.updateUserInfo({avatar: avatarRes.data.url, avatarKey: avatarPath})
...
const fetchQualification = async () => {
  try {
    ...
    if (qualification.value?.certificateImageKey) {
      const urlRes = await getQualificationCertificateUrl()
      qualification.value.certificateImage = urlRes.data.url
    }
  } catch (error) {
    // 未提交过资质，忽略错误
  ...
  await verifyFormRef.value.validate(async (valid) => {
    ...
    try {
      ...
      if (certificateFile.value) {
        ...
        certificateImageUrl = uploadRes.data.objectKey
      }
