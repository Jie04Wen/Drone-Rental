        ...
        <div v-if="faultList.length > 0" class="fault-list">
          <div v-for="fault in faultList" :key="fault.id" class="fault-item">
            <div class="fault-header">
              ...
              <StatusTag :text="getFaultStatusText(fault.auditStatus)" :type="getFaultStatusType(fault.auditStatus)"
                         size="small"/>
            </div>
    ...
    try {
      ...
      for (const file of imageList.value) {
        if (file.raw) {
          ...
          imageUrls.push(res.data.objectKey)
        }
      }
      ...
      ElMessage.success({ message: '报修申请已提交', duration: 1000 })
