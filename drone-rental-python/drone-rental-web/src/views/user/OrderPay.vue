    ...
    <div v-if="order" class="pay-content">
      <GlassCard class="order-summary">
        ...
        <div class="total-amount">
          <span>应付金额</span>
          <span class="amount">¥{{ money(order.payableAmount) }}</span>
        </div>
      </GlassCard>
      ...
      <GlassCard class="payment-methods">
        ...
        <div class="pay-action">
          ...
          <div class="countdown expired" v-else>支付有效期已结束</div>
          <el-button
              type="primary"
              size="large"
              :loading="paying"
              ...
              @click="handlePay"
          >
            确认支付 ¥{{ money(order.payableAmount) }}
          </el-button>
        </div>
      </GlassCard>
...
const fetchOrderDetail = async () => {
  ...
  try {
    ...
    paymentDeadlineMs.value = Date.now() + Number(order.value.remainingPaymentSeconds || 0) * 1000
    syncCountdown()
    if (countdown.value <= 0) {
      ElMessage.warning('订单已超过支付有效期')
      router.push('/orders')
      return
    }
    startCountdown()
...
const syncCountdown = () => {
  countdown.value = Math.max(Math.ceil((paymentDeadlineMs.value - Date.now()) / 1000), 0)
}
...
const handlePay = async () => {
  ...
  paying.value = true
  try {
    const res = await payOrder(order.value.id, selectedMethod.value, deliveryAddress.value.trim())
    const data = res?.data

    // 真实支付宝渠道：后端返回自动提交表单，注入并提交跳转
    if (data && data.channel === 'alipay_form' && data.form) {
      if (alipayFormContainer.value) {
        alipayFormContainer.value.innerHTML = data.form
        await nextTick()
        const form = alipayFormContainer.value.querySelector('#alipay_form')
        if (form) {
          form.submit()
          return
        }
      }
      ElMessage.error('跳转支付宝失败，请重试')
      return
    }
