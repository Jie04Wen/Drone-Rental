<template>
  <div class="order-detail-page" v-loading="loading">
      ...
      <div class="detail-main">
        ...
        <GlassCard title="费用明细" class="price-card">
          <div class="price-list">
            ...
            <div v-if="order.rentalRefundAmount > 0" class="price-item">
              <span v-if="order.faultFullRefund">
                {{ order.depositStatus === 0 ? '预计故障全额退租金' : '故障全额退租金' }}
              </span>
              <span v-else>{{ order.depositStatus === 0 ? '预计退租金' : '已退租金' }}（{{
                  order.unusedRentalDays
                }}天）</span>
              <span>-¥{{ Number(order.rentalRefundAmount).toFixed(2) }}</span>
            </div>
          </div>
          <el-divider/>
          <div class="price-total">
            <span>实付金额</span>
            <span class="total-value">¥{{ Number(order.payableAmount || 0).toFixed(2) }}</span>
          </div>
        </GlassCard>
      ...
      <div class="detail-sidebar">
        ...
        <GlassCard title="订单信息">
          <div class="info-list">
            ...
            <div v-if="order.returnAppliedTime" class="info-item">
              <span class="info-label">申请退租</span>
              <span class="info-value">{{ formatDateTime(order.returnAppliedTime) }}</span>
            </div>
            <div v-if="order.returnExpressNo" class="info-item">
              <span class="info-label">寄回物流</span>
              <span class="info-value">{{ order.returnExpressCompany }} {{ order.returnExpressNo }}</span>
            </div>
            <div v-if="order.returnShippedTime" class="info-item">
              <span class="info-label">寄回时间</span>
              <span class="info-value">{{ formatDateTime(order.returnShippedTime) }}</span>
            </div>
          </div>
        </GlassCard>
        ...
        <GlassCard class="action-card">
          <div class="action-buttons">
            ...
            <template v-else-if="order.orderStatus === 7">
              <el-button type="primary" size="large" @click="shipmentDialogVisible = true">填写寄回物流</el-button>
            </template>

            <template v-else-if="order.orderStatus === 8">
              <el-button disabled size="large">等待商家收货验机</el-button>
            </template>

            <!-- 已归还 (status=4) 显示评价按钮 -->
            <template v-else-if="order.orderStatus === 4">
              <el-button v-if="!order.hasComment" type="primary" size="large" @click="openCommentDialog">去评价
              </el-button>
    ...
    <el-dialog v-model="commentDialogVisible" title="评价订单" width="500px" :close-on-click-modal="false">
      <el-form ref="commentFormRef" :model="commentForm" :rules="commentRules" label-position="top">
        <el-form-item label="评分" prop="rating">
          <el-rate v-model="commentForm.rating" :colors="['#F59E0B', '#F59E0B', '#22C55E']" show-text
    ...
    <el-dialog v-model="shipmentDialogVisible" title="填写寄回物流" width="500px" :close-on-click-modal="false">
      <el-form label-width="100px">
        <el-form-item label="快递公司" required>
          <el-input v-model="shipmentForm.expressCompany" maxlength="100" placeholder="例如：顺丰速运"/>
        </el-form-item>
        <el-form-item label="快递单号" required>
          <el-input v-model="shipmentForm.expressNo" maxlength="100" placeholder="请输入寄回快递单号"/>
        </el-form-item>
      </el-form>
      <template #footer>
        <el-button @click="shipmentDialogVisible = false">取消</el-button>
        <el-button type="primary" :loading="shipmentSubmitting" @click="submitShipment">确认寄回</el-button>
      </template>
    </el-dialog>
  </div>
</template>
...
import {getOrderDetail, cancelOrder, confirmReceive, applyReturn, submitReturnShipment, commentOrder} from '@/api/order'
...
const shipmentForm = reactive({expressCompany: '', expressNo: ''})
...
const timelineItems = computed(() => {
  ...
  if (order.value.orderStatus === 6) {
    return [
      {title: '提交订单', time: formatDateTime(order.value.createdTime), completed: true},
      {title: '支付成功', time: formatDateTime(order.value.payTime), completed: true},
  ...
  const items = [
    {title: '提交订单', time: formatDateTime(order.value.createdTime), completed: true},
    {title: '支付成功', time: formatDateTime(order.value.payTime), completed: order.value.orderStatus >= 1},
    {title: '商家发货', time: formatDateTime(order.value.shipTime), completed: order.value.orderStatus >= 2},
    {title: '确认收货', time: formatDateTime(order.value.receiveTime), completed: order.value.orderStatus >= 3},
    {title: '申请退租', time: order.value.returnAppliedTime, completed: [4, 7, 8].includes(order.value.orderStatus)},
    {title: '寄回设备', time: order.value.returnShippedTime, completed: [4, 8].includes(order.value.orderStatus)},
    {title: '验机归还', time: order.value.depositSettledTime, completed: order.value.orderStatus === 4}
  ]
...
const handleCancel = async () => {
  try {
    await ElMessageBox.prompt('请输入取消原因', '取消订单', {
      ...
      ElMessage.success({ message: '订单已取消', duration: 1000 })
      fetchOrderDetail()
    })
  } catch {
  }
...
const handleReceive = async () => {
  try {
    ...
    ElMessage.success({ message: '已确认收货', duration: 1000 })
    fetchOrderDetail()
  } catch {
  }
}
...
const handleReturn = async () => {
  try {
    ...
    ElMessage.success({ message: '已申请退租，请尽快寄回设备', duration: 1000 })
    fetchOrderDetail()
  } catch {
  }
}
...
const submitShipment = async () => {
  if (!shipmentForm.expressCompany.trim() || !shipmentForm.expressNo.trim()) {
    ElMessage.warning('请填写寄回快递公司和快递单号')
    return
  }
  shipmentSubmitting.value = true
  try {
    await submitReturnShipment(order.value.id, {
      expressCompany: shipmentForm.expressCompany.trim(),
      expressNo: shipmentForm.expressNo.trim()
    })
    ElMessage.success({ message: '寄回物流已提交，等待商家收货验机', duration: 1000 })
...
const submitComment = async () => {
  ...
  await commentFormRef.value.validate(async (valid) => {
    ...
    try {
      ...
      ElMessage.success({ message: '评价成功', duration: 1000 })
