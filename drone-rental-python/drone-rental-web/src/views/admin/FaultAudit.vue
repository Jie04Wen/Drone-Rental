  ...
  <div class="fault-audit-page">
    ...
    <GlassCard class="table-card">
      <el-table :data="faultList" v-loading="loading" style="width: 100%">
        ...
        <el-table-column label="审核状态" width="120" align="center" header-align="center">
          <template #default="{ row }">
            <StatusTag :text="getAuditStatusText(row.auditStatus)" :type="getAuditStatusType(row.auditStatus)"
                       size="small"/>
          </template>
        </el-table-column>
        <el-table-column label="提交时间" width="180" align="center" header-align="center">
          <template #default="{ row }">
            <span class="submitted-time">{{ row.createdTime || '-' }}</span>
          </template>
        </el-table-column>
    ...
    <el-dialog v-model="detailVisible" title="故障报修详情" width="700px">
      <div v-if="currentFault" class="fault-detail">
        <el-descriptions :column="2" border>
          ...
          <el-descriptions-item label="审核状态">
            <StatusTag :text="getAuditStatusText(currentFault.auditStatus)"
                       :type="getAuditStatusType(currentFault.auditStatus)"/>
          </el-descriptions-item>
          <el-descriptions-item label="提交用户">{{
              currentFault.userName || currentFault.userId
            }}
          </el-descriptions-item>
          ...
          <el-descriptions-item label="故障时间">{{
              currentFault.faultTime || currentFault.createdTime
            }}
          </el-descriptions-item>
          <el-descriptions-item label="故障描述" :span="2">{{ currentFault.faultDescription }}</el-descriptions-item>
          <el-descriptions-item v-if="currentFault.auditStatus !== 0" label="审核备注" :span="2">
            {{ currentFault.auditRemark || '-' }}
          </el-descriptions-item>
          <el-descriptions-item v-if="currentFault.auditStatus !== 0" label="审核时间">{{
              currentFault.auditTime || '-'
            }}
          </el-descriptions-item>
        </el-descriptions>
      ...
      <template #footer>
        ...
        <el-button v-if="currentFault?.auditStatus === 0" type="primary"
                   @click="handleAudit(currentFault); detailVisible = false">去审核
        </el-button>
      </template>
    </el-dialog>
...
import {getFaultList, getAdminFaultDetail, getAdminFaultImageUrls, auditFault} from '@/api/maintenance'
...
const handleViewDetail = async (row) => {
  try {
    ...
    if (currentFault.value?.faultImageKeys) {
      const imageRes = await getAdminFaultImageUrls(row.id)
      currentFault.value.faultImages = imageRes.data.urls.join(',')
    }
...
const handleConfirmAudit = async () => {
  ...
  try {
    ...
    ElMessage.success({ message: '审核完成', duration: 1000 })
