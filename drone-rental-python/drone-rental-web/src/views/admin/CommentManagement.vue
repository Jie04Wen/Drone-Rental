  ...
  <div class="comment-management-page">
    ...
    <GlassCard class="table-card">
      <el-table :data="commentList" v-loading="loading" style="width: 100%">
        ...
        <el-table-column label="评分" width="140" align="center" header-align="center">
          <template #default="{ row }">
          ...
          <template #default="{ row }">
            <div class="status-stack">
              <StatusTag
                ...
                size="small"
              />
              <StatusTag v-if="row.status === 0" text="已屏蔽" type="danger" size="small" />
            </div>
          </template>
        </el-table-column>
        <el-table-column label="评价时间" width="180" align="center" header-align="center">
          <template #default="{ row }">
        ...
        <el-table-column label="操作" width="260" align="center" header-align="center">
          <template #default="{ row }">
            <div class="table-actions">
              ...
              <el-button v-else text type="success" @click="handleToggleStatus(row, 1)"
                >显示
              </el-button>
    ...
    <el-dialog v-model="replyVisible" title="回复评价" width="600px">
      ...
      <template #footer>
        ...
        <el-button type="primary" :loading="replying" @click="handleConfirmReply"
          >提交回复</el-button
        >
      </template>
    </el-dialog>
...
import { getAdminComments, replyComment, deleteComment, updateCommentStatus } from '@/api/comment'
...
const handleConfirmReply = async () => {
  ...
  try {
    ...
    ElMessage.success({ message: '回复成功', duration: 1000 })
...
const handleDelete = async (row) => {
  try {
    ...
    ElMessage.success({ message: '评价删除成功', duration: 1000 })
    fetchCommentList()
  } catch { /* empty */ }
}

const handleToggleStatus = async (row, status) => {
  const action = status === 0 ? '屏蔽' : '显示'
  try {
    await ElMessageBox.confirm(`确定要${action}该评价吗？`, `${action}确认`, {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      type: 'warning',
    })
