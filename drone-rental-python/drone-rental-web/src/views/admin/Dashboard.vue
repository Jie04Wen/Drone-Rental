          ...
          <div class="recent-list">
            <div v-for="order in recentOrders" :key="order.id" class="recent-item">
              ...
              <div class="recent-meta">
                <StatusTag :text="getOrderStatusText(order.status)" :type="getOrderStatusType(order.status)"
                           size="small"/>
...
<script setup>
import {ref, onMounted, onActivated, onBeforeUnmount, markRaw} from 'vue'
...
const todoItems = ref([
  ...
  },
  {
    ...
    path: '/admin/maintenance?status=0'
  }
])
...
const fetchTodoItems = async () => {
  try {
    ...
    const data = res.data || {}
    todoItems.value[0].count = data.audit || 0
    todoItems.value[1].count = data.ship || 0
    todoItems.value[2].count = data.returnRequested || 0
    todoItems.value[3].count = data.returnShipped || 0
    todoItems.value[4].count = data.maintenance || 0
  } catch (error) {
