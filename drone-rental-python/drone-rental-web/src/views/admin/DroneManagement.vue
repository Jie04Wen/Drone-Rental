  ...
  <div class="drone-management-page">
    ...
    <GlassCard class="table-card">
      <el-table :data="droneList" v-loading="loading" style="width: 100%">
        <el-table-column label="设备信息" min-width="250">
          <template #default="{ row }">
            <div class="drone-info">
              <el-image :src="getImageUrl(row.image)" fit="cover"
        ...
        <el-table-column label="日租金" width="100" align="center" header-align="center">
          <template #default="{ row }">
            <span class="price">¥{{ row.pricePerDay }}</span>
          </template>
        </el-table-column>
        <el-table-column label="押金" width="100" align="center" header-align="center">
          <template #default="{ row }">
            <span>¥{{ row.deposit ?? 0 }}</span>
          </template>
        </el-table-column>
        <el-table-column label="库存" width="80" align="center" header-align="center">
          <template #default="{ row }">
            <span :class="{ 'stock-low': row.stock < 3 }">{{ row.stock }}</span>
          </template>
        </el-table-column>
        <el-table-column label="设备状态" width="110" align="center" header-align="center">
          <template #default="{ row }">
            <StatusTag :text="getStatusText(row.status)" :type="getStatusType(row.status)" size="small"/>
          </template>
        </el-table-column>
        <el-table-column label="上架状态" width="110" align="center" header-align="center">
          <template #default="{ row }">
            <StatusTag
        ...
        <el-table-column label="租赁次数" width="100" align="center" header-align="center">
          <template #default="{ row }">
            <span>{{ formatRentCount(row.rentCount) }}</span>
          </template>
        </el-table-column>
        <el-table-column label="操作" width="220" align="center" header-align="center">
          <template #default="{ row }">
            <div class="action-cell">
              ...
              <el-button text type="primary" size="small" @click="handleStock(row)">库存</el-button>
              <el-button
                  text
                  size="small"
          ...
          <div class="image-upload-wrapper">
            ...
            >
              <div v-if="form.image" class="image-preview">
                <el-image :src="imagePreviewUrl || getImageUrl(form.image)" fit="cover"/>
...
const handleEdit = (row) => {
  ...
  Object.assign(form, row, {image: row.imageKey || row.image || ''})
...
const handleImageUpload = async (options) => {
  ...
  try {
    const res = await upload('/common/upload?category=drone', file)
    form.image = res.data.objectKey
    imagePreviewUrl.value = res.data.url
    ElMessage.success({ message: '图片上传成功', duration: 1000 })
  } catch (error) {
  ...
  await formRef.value.validate(async (valid) => {
    ...
    try {
      if (isEdit.value) {
        ...
        ElMessage.success({ message: '新增成功', duration: 1000 })
      }
...
const handleSaveStock = async () => {
  ...
  try {
    ...
    ElMessage.success({ message: '库存已更新', duration: 1000 })
...
const handleToggleShelf = async (row) => {
  ...
  try {
    ...
    ElMessage.success({ message: '${action}成功', duration: 1000 })
    fetchDroneList()
  } catch { /* empty */ }
}
