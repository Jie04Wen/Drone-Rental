  ...
  <div class="audit-management-page">
    ...
    <GlassCard class="table-card">
      ...
      <el-table :data="auditList" v-loading="loading" style="width: 100%">
        ...
        <el-table-column label="资质证书" width="120" align="center" header-align="center">
          <template #default="{ row }">
            <el-image
        ...
        <el-table-column label="有效期" min-width="210" align="center" header-align="center">
          <template #default="{ row }">
            <span class="date-range">{{ row.validStartDate }} ~ {{ row.validEndDate }}</span>
          </template>
        </el-table-column>
        ...
        <el-table-column label="提交时间" width="180" align="center" header-align="center">
          <template #default="{ row }">
            <span class="time-cell">{{ formatDateTime(row.createdTime) }}</span>
          </template>
        </el-table-column>
      ...
      <div v-if="currentAudit" class="audit-detail">
        <div class="detail-section">
          ...
          <el-descriptions :column="2" border>
            ...
            <el-descriptions-item label="有效期">{{ currentAudit.validStartDate }} ~ {{
                currentAudit.validEndDate
              }}
            </el-descriptions-item>
            <el-descriptions-item label="提交时间" :span="2">{{
                formatDateTime(currentAudit.createdTime)
              }}
            </el-descriptions-item>
          </el-descriptions>
        </div>
        ...
        <div v-if="currentAudit.auditStatus !== 0" class="detail-section">
          ...
          <el-descriptions :column="2" border>
            <el-descriptions-item label="审核状态">
              <StatusTag :text="getAuditStatusText(currentAudit.auditStatus)"
                         :type="getAuditStatusType(currentAudit.auditStatus)"/>
            </el-descriptions-item>
...
import {getPendingAudits, approveAudit, rejectAudit, getAuditStats, getAuditCertificateUrl} from '@/api/admin'
...
const handleViewDetail = async (row) => {
  ...
  if (row.certificateImageKey) {
    const res = await getAuditCertificateUrl(row.id)
    currentAudit.value.certificateImage = res.data.url
  }
...
const handleApprove = async (row) => {
  try {
    ...
    ElMessage.success({ message: '审核通过', duration: 1000 })
...
const handleReject = async (row) => {
  try {
    ...
    ElMessage.success({ message: '已拒绝', duration: 1000 })
    detailVisible.value = false
    fetchAuditList()
  } catch {
  }
