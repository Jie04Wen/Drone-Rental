      ...
      <div class="user-header__container">
        <div class="user-header__logo" @click="router.push('/')">
          <div class="user-header__logo-icon">
            <svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M12 2L4 7V17L12 22L20 17V7L12 2Z" stroke="currentColor" stroke-width="2"
        ...
        <nav class="user-header__nav">
          ...
          >
            ...
            <span class="user-header__nav-label">{{ item.title }}</span>
          </router-link>
        </nav>
          ...
          <template v-if="authStore.isLoggedIn">
            ...
            <el-badge :value="unreadCount" :hidden="unreadCount === 0" :max="99" class="notification-badge">
              <el-popover placement="bottom-end" :width="320" trigger="click">
                <template #reference>
                  <el-button text class="notification-bell">
                    <el-icon :size="20">
                      <Bell/>
                    </el-icon>
                  </el-button>
                ...
                <div class="notification-panel">
                  ...
                  <div class="notification-panel__list" v-if="notifications.length">
                    <div v-for="(n, i) in notifications" :key="i" class="notification-panel__item">
                      <div class="notification-panel__item-title">{{ n.title }}</div>
                      <div class="notification-panel__item-content">{{ n.content }}</div>
                      <div class="notification-panel__item-time">{{ n.createdTime }}</div>
                    </div>
                  </div>
                  <div v-else class="notification-panel__empty">暂无通知</div>
                </div>
              </el-popover>
            </el-badge>
      ...
      <div v-if="mobileMenuOpen" class="user-mobile-menu">
        ...
        <div class="user-mobile-menu__content">
          <div class="user-mobile-menu__header">
            <span>菜单</span>
            <el-icon @click="mobileMenuOpen = false">
              <Close/>
            </el-icon>
          </div>
    ...
    <footer class="user-footer">
      <div class="user-footer__container">
        ...
        <div class="user-footer__links">
          ...
          <div class="user-footer__links-group">
            <h4>帮助支持</h4>
            <router-link to="/rental-guide">租赁指南</router-link>
...
// WebSocket 通知
const {connected, notifications, unreadCount, markRead: markAllRead} = inject('notifications', {
  connected: ref(false),
  notifications: ref([]),
  unreadCount: ref(0),
  markRead: () => {
...
const handleLogout = async () => {
  try {
    ...
    ElMessage.success({ message: '已退出登录', duration: 1000 })
    router.push('/')
  } catch {
    // 取消退出
  }
